...
const DyvixMarquee = React.forwardRef<HTMLDivElement, DyvixMarqueeProps>(
  ({ children, className }, ref) => {
    ...
    const [displayItems, setDisplayItems] = React.useState<{originalItems: React.ReactNode[], duplicateItems: React.ReactNode[]} | null>(
      {originalItems: [], duplicateItems: []}
    );
    ...
    React.useLayoutEffect(() => {
      ...
      const intialchildeNodes = childeNodes.slice(
        0,
      ...
      const getCurrentWidth = () => {
        ...
        if (ogContentRef.current) {
          ...
          const rawGap = computedStyle.gap || computedStyle.columnGap || '0px';
      ...
      outer: for (let i = 0; i < fullSetMultiplier; i++) {
        for (let j = 0; j < intialchildeNodes.length; j++) {
          ...
          const effectiveWidth = child.getBoundingClientRect().width + gapValue;
        ...
        <div {...props}>
          <div className="dyvix-marquee-track" ref={trackRef}>
            <div className="dyvix-marquee-content" ref={ogContentRef}>
              {displayItems?.originalItems.length ? displayItems.originalItems : children}
            </div>
            <div className="dyvix-marquee-content" aria-hidden="true">
              {displayItems?.duplicateItems.length ? displayItems.duplicateItems : children}
            </div>
          </div>
        </div>
      </div>
